...
interface SelectContentStyles extends React.CSSProperties {
  '--radix-select-content-transform-origin': string;
  '--radix-select-content-available-width': string;
  '--radix-select-content-available-height': string;
  '--radix-select-trigger-width': string;
  ...
  return (
    ...
    >
      <div
        ...
        {...props}
        className={cn(
          'flex size-full min-h-10 items-center justify-between whitespace-nowrap rounded-md border border-input bg-transparent px-3 py-2 text-sm shadow-sm ring-offset-background focus:outline-none focus:ring-1 focus:ring-ring [&>span]:line-clamp-1',
...
>(
  (
    ...
    if (!value.length || !firstRendered) {
      return (
        <span className="pointer-events-none text-xl text-muted-foreground">
          {placeholder}
        </span>
      );
    }
