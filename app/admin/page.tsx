...
export default function page() {
  ...
  const isAuthorized = user?.emailAddresses[0]?.emailAddress === process.env.NEXT_PUBLIC_ADMIN_EMAIL;

  if (!isAuthorized) {
    return <div>Unauthorized access 
       <SignedOut>
              <SignInButton />
            </SignedOut>
            <SignedIn>
  ...
  const cards = unApprovedResources?.map(resource => ({
    _id:resource._id,
  ...
  return (
    <main>
      ...
      <h1 className="text-2xl font-bold mb-4">Admin: Unapproved Resources</h1>
